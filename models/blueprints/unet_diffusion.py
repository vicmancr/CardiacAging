...
class UNetModel(nn.Module):
    ...
    def __init__(
        ...
        if self.num_covariates == 1:
            self.embedding_func = lambda tstep, dim, covars: timestep_embedding(tstep, dim, self.emb_max_period)
        else:
            self.embedding_func = lambda tstep, dim, covars: covariates_embedding(th.stack([tstep, *covars]), dim, self.emb_max_period)
        ...
        self.time_embed = nn.Sequential(
            linear(self.model_channels * self.num_covariates, time_embed_dim),
            nn.SiLU(),
...
class EncoderUNetModel(nn.Module):
    ...
    def __init__(
        ...
        if self.num_covariates == 1:
            ...
            self.embedding_func = lambda tstep, dim, covars: covariates_embedding([tstep, *covars], dim, self.emb_max_period)
